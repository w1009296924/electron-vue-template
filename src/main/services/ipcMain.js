  ...
  Mainfunc(IsUseSysTitle) {
    ...
    ipcMain.handle("open-todo-win", (event, arg) => {
      ...
      if (data.length > 0) {
        ...
        ipcMain.on("todo-win-move", (event, res) => {
          childWin && childWin.setPosition(res.appX, res.appY);
        });
        ipcMain.on("todo-win-moved", (event, res) => {
          const bound = childWin.getBounds();
          bound.left = bound.x;
          bound.top = bound.y;
          bound.right = bound.x + bound.width;
        ...
        childWin.on("closed", () => {
          ...
          ipcMain.removeAllListeners("todo-win-move");
          ipcMain.removeAllListeners("todo-win-moved");
          let index = cidArray.indexOf(cidJson);
          if (index > -1) {
            cidArray.splice(index, 1);
          }
