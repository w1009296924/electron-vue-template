const task = {
  state: {
    nowTask: null,
    taskArray: [
      {
        state: "develop",
        demandNo: "UT-WLJR-2022-0912",
        taskName:
          "UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）",
        application: "mspmk-cli-credit",
        branch: "feature_UT-WLJR-2022-0912_M301",
        fdevUnit: "DEFV-2022-09-06-00011",
        ipmpUnit: "实施单元2022网络金融237-048",
        startTime: "2022/09/02",
        startTimeReal: "2022/09/02",
        sitTime: "2022/09/20",
        sitTimeReal: "2022/09/20",
        uatTime: "2022/09/20",
        uatTimeReal: "2022/09/20",
        uatFinishTime: "2022/09/27",
        uatFinishTimeReal: "2022/09/27",
        fireTime: "2022/10/18",
        // fireTimeReal: "2022/10/18",
        workload: "0.4",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "develop",
        demandNo: "UT-WLJR-2022-1013",
        taskName:
          "UT-WLJR-2022-1013-手机银行存款业务体验优化需求(结构性)-mspmk-cli-structdeposits",
        application: "mspmk-cli-structdeposits",
        branch: "feature_UT-WLJR-2022-1013_M301",
        fdevUnit: "DEFV-2022-09-06-00011",
        ipmpUnit: "实施单元2022网络金融237-048",
        startTime: "2022/09/15",
        startTimeReal: "2022/09/15",
        sitTime: "2022/10/14",
        sitTimeReal: "2022/10/14",
        uatTime: "2022/10/14",
        uatTimeReal: "2022/10/14",
        uatFinishTime: "2022/10/27",
        uatFinishTimeReal: "2022/10/27",
        fireTime: "2022/11/12",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "develop",
        demandNo: "UT-WLJR-2022-1013",
        taskName:
          "UT-WLJR-2022-1013-手机银行存款业务体验优化需求(约定)-mspmk-cli-duedeposit",
        application: "mspmk-cli-duedeposit",
        branch: "feature_UT-WLJR-2022-1013_M301",
        fdevUnit: "DEFV-2022-09-06-00011",
        ipmpUnit: "实施单元2022网络金融237-048",
        startTime: "2022/09/15",
        startTimeReal: "2022/09/15",
        sitTime: "2022/10/14",
        sitTimeReal: "2022/10/14",
        uatTime: "2022/10/14",
        uatTimeReal: "2022/10/14",
        uatFinishTime: "2022/10/27",
        uatFinishTimeReal: "2022/10/27",
        fireTime: "2022/11/12",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "develop",
        demandNo: "UT-WLJR-2022-1013",
        taskName:
          "UT-WLJR-2022-1013-手机银行存款业务体验优化需求(纪念日)-mspmk-cli-memorialdeposit",
        application: "mspmk-cli-memorialdeposit",
        branch: "feature_UT-WLJR-2022-1013_M301",
        fdevUnit: "DEFV-2022-09-06-00011",
        ipmpUnit: "实施单元2022网络金融237-048",
        startTime: "2022/09/15",
        startTimeReal: "2022/09/15",
        sitTime: "2022/10/14",
        sitTimeReal: "2022/10/14",
        uatTime: "2022/10/14",
        uatTimeReal: "2022/10/14",
        uatFinishTime: "2022/10/27",
        uatFinishTimeReal: "2022/10/27",
        fireTime: "2022/11/12",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "sit",
        demandNo: "UT-WLJR-2022-0710",
        taskName:
          "UT-WLJR-2022-0710-手机银行财富待办微信通知及场景化开通提醒业务需求(约定)-mspmk-cli-duedeposit",
        application: "mspmk-cli-duedeposit",
        branch: "feature_UT-WLJR-2022-0710_M301",
        fdevUnit: "DEFV-2022-09-06-00013",
        ipmpUnit: "实施单元2022网络金融237-051",
        startTime: "2022/08/25",
        startTimeReal: "2022/08/25",
        sitTime: "2022/09/12",
        sitTimeReal: "2022/09/12",
        uatTime: "2022/09/28",
        uatTimeReal: "2022/09/28",
        uatFinishTime: "2022/09/31",
        uatFinishTimeReal: "2022/09/31",
        fireTime: "2022/11/12",
        // fireTimeReal: "2022/10/18",
        workload: "0.2",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "sit",
        demandNo: "KEJI-2022-1550",
        taskName:
          "KEJI-2022-1550-UI业务组件交接四阶段-全场景接入改造(约定)-mspmk-cli-duedeposit",
        application: "mspmk-cli-duedeposit",
        branch: "feature_KEJI-2022-1550_M301",
        fdevUnit: "DEFV-2022-09-06-00013",
        ipmpUnit: "实施单元2022网络金融237-051",
        startTime: "2022/09/21",
        startTimeReal: "2022/09/22",
        sitTime: "2022/10/13",
        sitTimeReal: "2022/10/13",
        uatTime: "2022/10/15",
        uatTimeReal: "2022/10/15",
        uatFinishTime: "2022/10/23",
        uatFinishTimeReal: "2022/10/25",
        fireTime: "2022/11/08",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "sit",
        demandNo: "UT-WLJR-2022-0761",
        taskName:
          "UT-WLJR-2022-0761-手机银行财富产品找类似需求(结构性存款)-mspmk-cli-structdeposits",
        application: "mspmk-cli-structdeposits",
        branch: "feature_UT-WLJR-2022-0761_M301",
        fdevUnit: "DEFV-2022-09-06-00014",
        ipmpUnit: "实施单元2022网络金融237-054",
        startTime: "2022/08/18",
        startTimeReal: "2022/08/18",
        sitTime: "2022/09/08",
        sitTimeReal: "2022/09/08",
        uatTime: "2022/09/08",
        uatTimeReal: "",
        uatFinishTime: "2022/09/17",
        uatFinishTimeReal: "",
        fireTime: "2022/09/27",
        // fireTimeReal: "2022/10/18",
        workload: "0.5",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "uat",
        demandNo: "UT-WLJR-2022-0678",
        taskName:
          "UT-WLJR-2022-0678-手机银行存款搜索等业务体验日常优化需求(通知)-mspmk-cli-noticedeposit",
        application: "mspmk-cli-structdeposits",
        branch: "feature_UT-WLJR-2022-0678_M301",
        fdevUnit: "DEFV-2022-09-06-00015",
        ipmpUnit: "实施单元2022网络金融237-056",
        startTime: "2022/07/19",
        startTimeReal: "2022/07/19",
        sitTime: "2022/08/17",
        sitTimeReal: "2022/08/17",
        uatTime: "2022/08/17",
        uatTimeReal: "2022/08/17",
        uatFinishTime: "2022/08/27",
        uatFinishTimeReal: "2022/08/27",
        fireTime: "2022/09/06",
        // fireTimeReal: "2022/10/18",
        workload: "1.2",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "uat",
        demandNo: "KEJI-2022-0752",
        taskName:
          "KEJI-2022-0752-A2团队二季度体验优化(约定)-mspmk-cli-duedeposit",
        application: "mspmk-cli-duedeposit",
        branch: "feature_KEJI-2022-0752_M305",
        fdevUnit: "DEFV-2022-09-06-00016",
        ipmpUnit: "实施单元2022网络金融237-058",
        startTime: "2022/07/25",
        startTimeReal: "2022/07/25",
        sitTime: "2022/08/12",
        sitTimeReal: "2022/08/12",
        uatTime: "2022/08/19",
        uatTimeReal: "2022/08/19",
        uatFinishTime: "2022/08/27",
        uatFinishTimeReal: "2022/08/27",
        fireTime: "2022/09/06",
        // fireTimeReal: "2022/10/18",
        workload: "0.3",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "rel",
        demandNo: "KEJI-2022-1682",
        taskName:
          "KEJI-2022-1682-手机银行人像认证安全加固场景接入(纪念日)-mspmk-cli-memorialdeposit",
        application: "mspmk-cli-memorialdeposit",
        branch: "feature_KEJI-2022-1682_M301",
        fdevUnit: "DEFV-2022-09-06-00017",
        ipmpUnit: "实施单元2022网络金融237-060",
        startTime: "2022/07/31",
        startTimeReal: "2022/07/31",
        sitTime: "2022/08/29",
        sitTimeReal: "2022/08/29",
        uatTime: "2022/09/02",
        uatTimeReal: "2022/09/02",
        uatFinishTime: "2022/09/05",
        uatFinishTimeReal: "2022/09/05",
        fireTime: "2022/09/09",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
      {
        state: "rel",
        demandNo: "KEJI-2022-1839",
        taskName:
          "KEJI-2022-1839-路由2.0改造弹窗问题修复(结构性)-mspmk-cli-structdeposits",
        application: "mspmk-cli-memorialdeposit",
        branch: "feature_KEJI-2022-1839_M301",
        fdevUnit: "DEFV-2022-09-06-00018",
        ipmpUnit: "实施单元2022网络金融237-061",
        startTime: "2022/08/11",
        startTimeReal: "2022/08/12",
        sitTime: "2022/08/28",
        sitTimeReal: "2022/08/28",
        uatTime: "2022/08/28",
        uatTimeReal: "2022/08/28",
        uatFinishTime: "2022/09/05",
        uatFinishTimeReal: "2022/09/05",
        fireTime: "2022/09/13",
        // fireTimeReal: "2022/10/18",
        workload: "0.1",
        fileDir:
          "D:\\liyw11归档目录\\2022年09月\\UT-WLJR-2022-0912-还款交易等联机反欺诈系统需求（手机信用卡）\\mission.txt",
      },
    ],
  },
  mutations: {
    SET_TASKARRAY: (state, taskArray) => {
      state.taskArray = taskArray;
    },
    SET_NOWTASK: (state, nowTask) => {
      // let [...newArray] = state.taskArray;
      // newArray.forEach((element) => {
      //   element.selected = element == nowTask;
      // });
      // state.taskArray = newArray;
      state.nowTask = nowTask;
    },
    SET_WORKLOAD: (state, [taskObj, workload]) => {
      let task = state.taskArray.find((element) => {
        return element.taskName == taskObj.taskName;
      });
      task.workload = workload;
    },
  },
  actions: {
    SelectTask({ commit }, data) {
      // let [...newArray] = state.taskArray;
      // newArray.forEach((element) => {
      //   element.selected = element == data;
      // });
      // commit("SET_TASKARRAY", newArray);
      commit("SET_NOWTASK", data);
    },
    // GenerateRoutes({ commit }, data) {
    //     return new Promise(resolve => {
    //         const roles = data
    //         let accessedRouters = []
    //         // 在这里当是管理员权限时,就给予所有的路由表
    //         if (roles === 'admin') {
    //             accessedRouters = asyncRoutes || []
    //         } else {
    //             // 根据需通过得到的权限于路由表中对比得到该用户应有的边栏
    //             accessedRouters = filterAsyncRouter(asyncRoutes, roles)
    //             // 根据产品类型再次处理路由表
    //         }
    //         commit('SET_ROUTERS', accessedRouters)
    //         resolve(accessedRouters)
    //     })
    // },
    // ResetRoutes({ commit }, data) {
    //     commit('RESET_ROUTERS', [])
    // }
  },
};

export default task;
